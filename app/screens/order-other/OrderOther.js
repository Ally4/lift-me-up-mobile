...
import { orderOtherStart, orderOtherSuccess, orderOtherFailure } from '../../features/auth/orderOtherSlice';
...
export default function OrderOther() {
  ...
  const handleOrderOther = async () => {
    if (!nameOfTest || !firstName || !lastName || !sex || !age || !accessPoint || !phoneNumber || !payment) {
      dispatch(orderOtherFailure('You miss something in your order.'));
      return;
    }
    dispatch(orderOtherStart());
    try {
      ...
      if ((response.data.status).toString() === '201') {
        dispatch(orderOtherSuccess({ user: response.data.user, token: response.data.token }));
        navigation.navigate('Main');
      } else {
        dispatch(orderOtherFailure('You miss something in your order.'));
      }
    } catch (error) {
      dispatch(orderOtherFailure('An error occurred during the order.'));
    }
  };
